...
- Work in the main repository checkout for ordinary Prompt Lab changes. Do not
  create a separate git worktree unless the user asks for one or an actual
  checkout conflict makes it necessary.
...
- Use the shared `CodeViewer` / `DiffViewer` components for read-only prompt,
  model, JSON, raw output, rendered prompt, and validation artifact displays.
  Keep custom syntax colors for Jinja and `<<MODEL>>` visually distinct from
  diff red/green, and keep code viewer backgrounds aligned with the app's light
  surfaces unless the surrounding UI explicitly uses a dark surface.
